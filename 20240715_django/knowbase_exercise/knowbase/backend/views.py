...
def check(request):
    ...
    author_exists = AuthorBasicInfo.objects.filter(username=username).exists()
    if author_exists:
        context = {
            'status':10001,
            'message':'Author alreay exists'
        }
    else:
        context = {
            'status':10002,
...
def post_article(request):
    ...
    Article.objects.create(
        ...
        author_id=request.session['backend']['id']
    )
    Category.objects.filter(pk=category_id).update(article_number=F('article_number')+1)
    AuthorBasicInfo.objects.filter(pk=request.session['backend']['id']).update(article_number = F('article_number')+1)
