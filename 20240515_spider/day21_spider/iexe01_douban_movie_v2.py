...
class DouBanSpider(object):
    ...
    @staticmethod
    def __save_file(file_name):
        file_writer = csv.writer(file_name)
        file_writer.writerow(['排名', '电影名', '评分', '评论数', '上映时间'])

        def write_file(list_info):
            file_writer.writerows(list_info)
    ...
    def main(self):
        ...
        file_name = open(self.__file_name, 'w', encoding='utf-8', newline='')
        file_save = self.__save_file(file_name)
