...
def do_get(http_client):
    ...
    with open('02-register.html', 'rb') as file:
        file_data = file.read()
...
def do_post(http_client,request_body):
    ...
    username = parsed_query.get('username')[0]
    password = md5(parsed_query.get('password')[0])
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if cursor.rowcount:
        db_result = '对不起，用户已经存在，<a href="javascript:history.back()">重新注册</a>'.encode()
    else:
        cursor.execute('INSERT users(username,password) VALUES(%s,%s)',[username,password])
        db_result = b'<h1>' + '注册成功'.encode() + b'OK</h1>'
    cursor.close()
    connection.close()
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    response = b'HTTP/1.1 200 OK\r\n'
